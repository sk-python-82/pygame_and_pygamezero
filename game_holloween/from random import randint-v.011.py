...
# Create a list of Rect objects for the flowers with random positions (excluding the pink flower)
flowers = [flower1.get_rect(topleft=(random.randint(0, width - 50), random.randint(0, height - 50))) for _ in range(9)]
...
pink_flower_collected = False  # To track if the pink flower has been collected
...
while running:
    ...
    # Check if the girl reaches the pink flower
    if not pink_flower_collected and girl_rect.colliderect(pink_flower_rect):
        pink_flower_collected = True
        flowers_collected += 1
    ...
    if countdown_timer == 0 or pink_flower_collected:
        girl_movable = False
    ...
    # Display the pink flower in the center of the screen (if not collected)
    if not pink_flower_collected:
        screen.blit(pink_flower, pink_flower_rect.topleft)
