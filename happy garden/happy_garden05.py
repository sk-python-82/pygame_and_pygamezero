...
def draw():
    ...
    if not game_over:
        ...
        screen.draw.text("Garden happy for :"+ \
                         str(time_elapsed)+ "seconds",topleft=(10,10),color="black")
    else:
         if not finalized:
             cow.draw()
             screen.draw.text("Garden happy for :"+ \
                         str(time_elapsed)+ "seconds",topleft=(10,10),color="black")
         if not garden_happy:
             screen.draw.text("Garden unhappy game over :",topleft=(10,10),color="black")
             finalized = True
         else:
             screen.draw.text("fangflower attack!!! game over :",topleft=(10,30),color="black")
             finalized = True
    return
...
def new_flower():
    ...
    flower_new.pos = randint(50,WIDTH-50),randint(180,HEIGHT-100)
    flower_list.append(flower_new)
    wilted_list.append("happy")
    return
    
def add_flower():             #recursive function
    global game_over
    if not game_over:
        new_flower()
...
def wilt_flower():
    global game_over,flower_list,wilted_list
    if not game_over:
        if flower_list:
...
wilt_flower()
def update():
    global score, game_over, fangflower_collision,time_elapsed,flower_list,fangflower_list
